...
replace_mode = 0; % if want to replace preweight data 
...
for data_file = 1:length(files)
    ...
    if isempty(char(post_date)) == 1 % no post-weigh data
        disp('There are no post-weights in this MTL data report')
    ...
    % usually just one site for each file.
    for site = 1:size(site_IDs,1)
        ...
        % 1. No MTL mass file exist - create one.
        % 2. MTL mass file exists, and includes all filters in the new report -
        % only post weight need to be added. If no post weights available 
        % for the current report, the 'pre & post weight' file was read 
        % before the 'pre weight' file. No action needed.
        % 3. MTL mass file exists, but doesn't include all filters in the
        % report - adding the pre (and possibly post) weight data to file.
                
        if  ~exist(mass_file,'file') % 1. No MTL mass file exist - create one.
                            
            fprintf('No MTL mass file exists for %s. Creating new MTL mass file.\n', site_IDs(site,:))

            % Index of samples for site_ID(i) in MTL mass report file
            ...
            % could be possible that first file found has both preweights and postweights
        ...
        else % MTL mass file exist. Read it.
            fprintf('MTL mass file found for %s \n', site_IDs(site,:))
            ...
            if isempty(newIDs_idx) % Scenario 2: file exist, and all filters included.
                
                if postweights_avail == 1 % adding post weight
                    massfile_idx = find(ismember(mass_file_table.FilterID,site_filterIDs) == 1);
                    % If post-weights are available
                    mass_file_table.Postweigh_Date(massfile_idx) = post_date;
                    mass_file_table.Postweight_avg_mg(massfile_idx) = raw_file.CorrectedPosttest(sample_rows(samples_site_idx)); % units = mg
                    mass_file_table.Postweight_std_ug(massfile_idx) = raw_file.PosttestStandardDeviation(sample_rows(samples_site_idx)); % unit = ug
                    mass_file_table.Postweight_NumReps(massfile_idx) = raw_file.PosttestRepetitions(sample_rows(samples_site_idx)); % unitless
                    mass_file_table.Net_Weight_ug(massfile_idx) = round((mass_file_table.Postweight_avg_mg(massfile_idx)  - mass_file_table.Preweight_avg_mg(massfile_idx) )*1000,1);
                else % sometimes need to update part of the preweight data 
                    if replace_mode == 1
                    massfile_idx = find(ismember(mass_file_table.FilterID,site_filterIDs) == 1);
                    mass_file_table(massfile_idx,:) = []; % delete exsiting data
                    ...
                    % Get Preweight info for this site from MTL report file
                    preweight_avgs = raw_file.CorrectedPretest(sample_rows(samples_site_idx)); % units = mg
                    preweight_stds = raw_file.PretestStandardDeviation(sample_rows(samples_site_idx)); % units = ug
                    preweight_reps = raw_file.PretestRepetitions(sample_rows(samples_site_idx));  % unitless
            ...
            else % Scenario 3: Filter IDs do NOT exist in MTL
                ...
                % Get Postweight info for this site from MTL report file   
                if postweights_avail == 1                  
                    postweight_date(1:size(samples_site_idx,1),1) = post_date;
                    postweight_avgs = raw_file.CorrectedPosttest(sample_rows(samples_site_idx)); % units = mg 
                    postweight_stds = raw_file.PosttestStandardDeviation(sample_rows(samples_site_idx)); % unit = ug
            ...
            if exist('site_analysisIDs','var')
                % combine all variable to a table in the same format as the
                % mass file:
            ...
            clear site_filterIDs_new site_barcodes samples_site_idx...
                  ...
                  site_postweigh_dates netweight mass_file massfile_idx ...
